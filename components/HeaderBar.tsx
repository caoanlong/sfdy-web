        ...
        <div className="w-full h-12 sm:h-16 fixed z-50 shadow bg-white dark:bg-gray-900">
            <div className="container h-full flex">
                ...
                <ul className="flex-1 h-full clear-both hidden lg:block">
                    <li className={`float-left h-full px-4 flex items-center ${typeId === '0' ? 'text-purple-500' : 'text-gray-600 hover:text-purple-500'}`}>
                        <Link href="/">首页</Link>
                    </li>
                    {
                        vodTypes.map((nav: VodType) => (
                            <li 
                                className={`float-left h-full px-4 flex items-center ${+typeId === nav.typeId ? 'text-purple-500' : 'text-gray-600 hover:text-purple-500'}`} 
                                key={nav.typeId}>
                ...
                </ul>
                {
                    showNavs && <div 
                        ...
                        <ul className="w-full absolute left-0 top-0 sm:top-16 bg-gray-100 dark:bg-gray-800 text-gray-600 shadow-md lg:hidden">
                            ...
                            </li>
                            {
                                vodTypes.map((nav: VodType) => (
                                    <li 
                                        className="container h-14 px-4 flex items-center" 
                                        key={nav.typeId} 
                ...
                <div className="w-64 h-full items-center hidden sm:flex">
                    <div 
                        ...
                        <input className="flex-1 h-full px-3 bg-transparent outline-none dark:text-white" type="text" placeholder="请输入关键字"/>
